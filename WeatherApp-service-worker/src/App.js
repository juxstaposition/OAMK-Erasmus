...
class App extends React.Component {
    constructor(props){
        ...
        this.citiesNames = CityList.map(item => item.name+ ", " + item.country);
    ...
    toggleWindow(param) {
        if ((param !== null && param !== "top")){
            ...
            console.log(this.citiesArr[i].icon);
            this.setState({
    ...
    async getWeather (val) {
        ...
        if (cityString) {
            ...
            if (city_found){
                ...
                const ApiCallWeather = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city[0]},${city[1]}&appid=${ApiKey}&units=metric`);
                ...
                if (dataTemp){
                    this.setState({
                        ...
                        saveButton: true,
                        icon: dataTemp.weather[0].icon,
                        error: undefined
                    });
                    
                    // I make another api call for fetching weather forecast
                    const ApiCallForecast = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${this.state.name},${this.state.country}&appid=${ApiKey}&units=metric`);
    ...
    async componentDidMount(){
        ...
        for (var i=0;i<this.citiesArr.length;i++){
            ...
            const dataTemp = await ApiCallWeather.json();
            const dataForecast = await ApiCallForecast.json();
            this.citiesArr[i].icon = dataTemp.weather[0].icon;
